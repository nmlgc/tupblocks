...
2. The Tupfile can't make decisions based on the pkg-config output, which would be required to e.g. fall back from system libraries to vendored ones.
...
-- Create a ConfigShape that represents the system-wide zlib and falls back
-- onto the vendored one if pkg-config couldn't find it.
